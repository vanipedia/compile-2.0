 ...
 */
Quote = MVC.Model.extend('quote', /* @Static */ {
  ...
  find_reference: function(params) {
    ...
    if (that.cache[params.ref]) {
      if (window.console) {
        console.info('Quote#find_reference: found submitted ref in Quote.cache. ' + params.ref);
      }
        ...
        if (resp.result !== 'Found') {
          ...
          if (!attr_search) {
            that.publish('warning', {
              msg: 'Ref not found for ' + params.ref
            });
          ...
          if (attr_search) {
            if (window.console) {
              console.info('Quote#find_reference: updating ' + params.attr + ' => ' + that.cache[params.ref][params.attr]);
            }
    ...
    function connection_error(xmlreq, text, error) {
      ...
      if (navigator.platform == 'MacIntel') {
        ...
        that.publish('connection_error', {
          ajax: ajax,
          msg: 'Vaniquotes server is unreachable, please wait a minute and submit your quote again.'
        });
      }
    }
    $(document).ajaxStart(function() {
      that.publish('ajax', {
        type: 'start',
        msg: 'Quote being processed...'
      });
    });
    $(document).ajaxStop(function() {
      that.publish('ajax', {
        type: 'end',
        msg: 'Quote request done'
      });
    });
  },
  /**
  ...
  need_section: function(ref) {
    ...
    if (/^(SB \d+.\d+.\d+(?:-\d+)?|BG \d+.\d+(?:-\d+)?|CC (Adi|Madhya|Antya) \d+.\d+(?:-\d+)?|NOI \d+|ISO \d+|MM \d+|NBS \d+|LOB \d+)$/.test(ref)) {
      if (window.console) {
        console.info('Quote#need_section: quote ' + ref + ' needs trans/purport section');
      }
      return true;
    } else {
      return false;
  ...
  update_tips: function(quote) {
    ...
    if (quote.type === 'new') {
      quote_tips.push(this.tips_db.insert_new);
    }
  ...
  update_section: function(quote) {
    ...
    if (this.need_section(quote.link)) { // check if this quote requires sections
      if (window.console) {
        console.info('Quote#update_section: finding section(s) for ' + quote.link);
      }
      ...
      if (quote.trans && quote.purport) {
        ...
        if (quote.type !== 'new') {
          if (window.console) {
            console.warn('This quote ' + quote.link + ' does not contain a proper section');
          }
        }
      }
      if (quote.trans || quote.purport) {
  ...
  request_verse: function(quote, verse) {
    ...
    if (typeof quote === 'string') {
      quote = Compilation.find_in_db(quote, 'q');
    }
    if (window.console) {
      console.log("Is this a verse?\n" + v);
    }
    if (quote.verses === false) {
      quote.verses = [];
    }
    this.publish('verse_request', {
      verse: v,
      id: quote.verses.length,
      quote: quote.id
    });
  ...
  clean: function(quote) {
    ...
    if (quote.type == 'new') {
      quote.text = BaltoUni(quote.text);
    }
  ...
  init: function(quote_obj) {
    if (quote_obj === undefined) {
      if (window.console) {
        console.error('Error creating new quote with quote_obj: ' + quote_obj);
      }
      return;
    }
    ...
    if (this.link) {
      this.id = this.link.replace(/\W/g, '') + '_' + this_sec_index;
    }
    ...
    this.publish('created', this); /**** Helper Functions ****/
    ...
    function check_missing_attr() {
      $.each(['link', 'parent', 'index'], function(i, attr) {
        if (!that[attr]) {
          if (attr == 'link') {
            if (window.console) {
              console.error('Quote.init#check_missing_attr: Missing link!' + attr);
            }
            return false;
          } else {
            if (window.console) {
              console.warn('Quote.init#check_missing_attr: Missing ' + attr);
            }
            find_attr(attr);
          }
        }
    ...
    function find_attr(attr) {
      var resp;
      if (window.console) {
        console.log('In Quote.init#find_attr, checking Quote.cache[' + that.link + '] = ');
        console.dir(Quote.cache[that.link]);
      }
      if (Quote.cache[that.link]) {
        if (window.console) {
          console.info('In Quote.init#find_attr, updating ' + attr + ' with ' + Quote.cache[that.link][attr]);
        }
        ...
        if (!that.link) {
          ...
          if (window.console) {
            console.error('Quote.init#find_attr: Missing that.link to query db for missing ' + attr);
          }
        } else {
          if (window.console) {
            console.warn('Quote.init#find_attr: ' + attr + ' not found in Quote.cache for ' + that.link + '. Submiting request to Quote.find_reference');
          }
          // Clean link since it most probably has underscores instead of spaces
          Quote.find_reference({
            quote: that,
            ref: that.link,
            type: 'title',
            attr: attr
    ...
    function transform_link_text() {
      if (!that.link_text) {
        that.link_text = that.link.replace(/_/g, ' ');
      }
