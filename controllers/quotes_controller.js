QuotesController = MVC.Controller.extend('quotes', /* @Static */ {
    ...
    enable_keybindings: function (q_obj) {
        $(document).bind('keydown', {
            quote: q_obj
        }, QuotesController.keybindings_event);
    },
    ...
    enable_edit_keybindings: function (q_obj) {
        ...
        $(q_obj).bind('keydown', {
            q: q_obj
        }, QuotesController.edit_view_keybindings_event);
    },
    ...
    mouseover: function (params) {
        ...
        if (!$(params.element).hasClass('edit_quote')) {
    ...
    "input#prabhupada_icon click": function (params) {
        ...
        if (edit_quote_text.value.length === edit_quote_text.selectionStart) {
            // set selection to begin of paragraph
            edit_quote_text.selectionStart = 0;
            edit_quote_text.selectionEnd = 0;
        }
    ...
    _append_quote: function (quote) {
        ...
        if ($('#' + section).length) {
            ...
            this.publish('report_error', {
                msg: 'Error rendering ' + quote.id + ' to ' + section
            });
            return;
        }
    },
    ...
    _transition_hilite: function (elem, color, time) {
        ...
        $(elem).removeClass('ui-widget-content').addClass('ui-state-highlight');
        setTimeout(function () {
            // $(elem).css('background-color', '');
            $(elem).removeClass('ui-state-highlight').addClass('ui-widget-content');
        }, time + '000');
    ...
    "rendered subscribe": function (elem) {
        var that;
        that = this;
        if ($(elem).hasClass('deleted_quote') || $(elem).hasClass('edit_quote')) {
            return;
        }
    ...
    "compilation.updated subscribe": function (params) {
        ...
        if (!$quote.hasClass('building_quote')) {
            $quote.addClass('q_updated ui-state-highlight').removeClass('q_new');
