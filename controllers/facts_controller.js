FactsController = MVC.Controller.extend('facts', /* @Static */ {
  key: {
    expressions: 69,
    // e
    categories: 67,
  ...
  "div.facts_tools input.add blur": function(params) {
    if ($('.cat_suggest_results').is(':hidden')) {
      this.reset_input(params.element);
    }
  },
  ...
  "div#copy_terms span.copy click": function(params) {
    ...
    if (all_terms === '') {
      this.publish('warning', {
        msg: 'You have no expressions to copy'
      });
      return;
    }
    if (window.console) {
  ...
  append: function(params) {
    ...
    if ($('#' + parent).length === 0) {
      if (window.console) {
        console.error('Error in FactsController.append: ' + parent + ' does not exist in dom');
      }
      return;
    }
    this.render({
  ...
  un_hilite: function(term) {
    if (!term) {
      if (window.console) {
        console.log('term missing in FactsController.un_hilite ' + term);
      }
      return;
    }
    var v, re;
    re = new RegExp('<span class="ui-state-highlight">' + term + '</span>', 'g');
  ...
  _render_facts: function(facts) {
    ...
    that = this;
    this.facts = facts;
    $('#facts').length ? options = {
      to: 'facts',
      action: 'facts'
    } : options = {
      bottom: 'compile_tools',
      action: 'facts'
    };
    this.render(options);
