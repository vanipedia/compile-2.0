...
/* @Static */
{
  compile_tools_menu_hover_options: {
        ...
        over: function() { CompileController.show_tools_menu(); }, // function = onMouseOver callback (REQUIRED)
        timeout: 500, // number = milliseconds delay before onMouseOut
        out: function() { CompileController.hide_tools_menu(); } // function = onMouseOut callback (REQUIRED)
    },
...
/* @Prototype */
{
  ...
  sort_section: function(section) {
    if($('#'+section).children().length > 1) {
      $('#'+section).children('.section, .sub_section').tsort({ attr: "sec_index" });
    }
  },
